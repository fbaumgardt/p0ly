import numpy as np, statsmodels.formula.api as sm, pandas as pd
from joblib import Parallel, delayed
from time import time as tm

def find_next(arr, el):
    """Cut array at first instance of el"""
    if len(arr) and arr[0] != el:
        arr = find_next(arr[1:], el)
    return arr


def find_lengths(arr, el, not_el=0):
    """Find lengths of consecutive instances of el"""
    res = []
    arr = find_next(arr, el)
    while len(arr):
        old = len(arr)
        arr = find_next(arr, not_el)
        res += [old - len(arr)]
        arr = find_next(arr, el)
    return res


def filter_columns(df, formula):
    return df.loc[:, [c for c in df.columns if c in formula + "Subject"]]


def sample_lmm(f, d, s, lmm_kwargs={}):
...
def channel_lmm(formula, data, design, ch_name="", n_jobs=1):
    ...
    res = Parallel(n_jobs=n_jobs)(
        delayed(sample_lmm)(formula, design.assign(EEG=data[:, s]), s) for s in range(data.shape[1]))
...
def group_lmm(epochs, formula, lmm_kwargs={}, n_jobs=1):
    ...
    design = filter_columns(pd.concat([e.metadata for e in epochs]), formula)
...
def get_trial_pos_in_block(block_type, block_index, df, pos=None):
    """
    Relative trial position by block
    :param block_type: Block type to scan
    :param block_index: Block index to scan
    :param df: Subject's metadata
    :param pos:
    :return: Array w/ trial position for specified block type and index
    """
...
def add_blockpercent(df, zippy=None):
    """
    Relative trial position for each block
    :param df: Subject's metadata
    :param zippy: Possible combination of block types and indices
...
def add_md(e):
    e.metadata = add_incorrect(
        add_delay(add_blockpercent(add_correctness(add_modality(add_subject(e.metadata, e.filename.split("/")[-2]))))))
    return e


def add_subblock(df):
    """
    Add sub-block index according to Debra's numbering scheme
    :param df: Subject's by-trial metadata
    :return: Modified metadata
    """
...
def add_model_family(df):
    return df.assign(model_family=df.apply(
        lambda d: "WM" if d.model_type in [15, 16, 17, 18] else "RL" if d.model_type in [3, 4, 11, 12, 13, 14, 19,
                                                                                         20] else "RLWM", axis=1))


#def add_model_params(df, params_dict):
