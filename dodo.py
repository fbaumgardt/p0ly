...
from config import fname, subjects, brainvision_subjects, epo_cfg, morlet_cfg, firwin_cfg, coh_cfg, glm_cfg, subj_prefix
...
def task_morlet():
    ...
    for subject in subjects:
        for lock in epo_cfg.locks:
            subject_epo = fname.subject_epo(subject=subject,lock=lock)
            subject_morlet = fname.subject_morlet(subject=subject,lock=lock,fmin=freqs[0],fmax=freqs[-1],nsteps=len(freqs),scale=scale)
            yield dict(
                name=f'{subject}-{lock}',
...
def task_coherence():
    ...
    get_freqs = lambda n: (morlet_cfg.freqs[0],morlet_cfg.freqs[-1]) if n=="cwt_morlet" else (firwin_cfg.freqs[0][0],firwin_cfg.freqs[-1][-1])
    for subject in subjects:
        for lock in epo_cfg.locks:
            subject_epo = fname.subject_epo(subject=subject,lock=lock)
            subjects_connectivity = [fname.subject_connectivity(subject=subject,lock=lock,fmin=get_freqs(n)[0],
